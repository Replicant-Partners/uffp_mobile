...
export interface ContextualInsight {
  type: "missing_driver" | "bias_warning" | "agent_recommendation" | "coverage_gap" | "correlation_risk";
...
export function analyzeMissingDrivers(forecast: Forecast): ContextualInsight[] {
  ...
  const categories = {
    ...
    technical: ["technical", "feasibility", "development", "implementation", "capability"],
...
export function detectBiases(forecast: Forecast): ContextualInsight[] {
  ...
  if (forecast.drivers.length > 0 && forecast.drivers[0].type === "binary") {
    insights.push({
      ...
      description: "Your first driver is a binary event. Be careful not to anchor too heavily on yes/no outcomes. Consider underlying continuous factors.",
      suggestedAction: "Add continuous drivers that influence this binary outcome"
    });
  }
  ...
  if (narrowDrivers.length > forecast.drivers.length * 0.6) {
    insights.push({
      ...
      suggestedAction: "Review your confidence intervals. Consider expanding ranges to reflect true uncertainty.",
      suggestedCommand: `/p`
    });
  }

  // Confirmation bias - no contradicting drivers
  const increasingDrivers = forecast.drivers.filter(d => d.direction === "increases").length;
  const decreasingDrivers = forecast.drivers.filter(d => d.direction === "decreases").length;
  ...
  if (forecast.drivers.length >= 3) {
    if (increasingDrivers === 0 || decreasingDrivers === 0) {
      insights.push({
        ...
        description: "All your drivers point in the same direction. Strong forecasts consider both supporting and opposing factors.",
...
export function recommendAgents(forecast: Forecast): ContextualInsight[] {
  ...
  for (const rec of agentRecommendations) {
    const matches = rec.patterns.some(pattern =>
      question.includes(pattern) || domain.includes(pattern)
    );
  ...
  if (attachedAgents.size === 0 && forecast.drivers.length > 0) {
    insights.push({
      ...
      description: "Research agents can continuously gather evidence and update your forecast. Start with @research_analyst for comprehensive analysis.",
...
export function analyzeDriverCoverage(forecast: Forecast): ContextualInsight[] {
  ...
  if (forecast.drivers.length < 2) {
    insights.push({
      ...
      description: "Complex forecasts benefit from multiple drivers. Consider breaking down your question into 3-5 key factors.",
      suggestedCommand: `/driver`
    });
  }
  ...
  if (driversWithoutEvidence.length > 0) {
    insights.push({
      ...
      description: "Attach research agents or add evidence to strengthen these drivers.",
    ...
    for (let j = i + 1; j < drivers.length; j++) {
      ...
      if (sharedWords.length >= 2) {
        insights.push({
          ...
          suggestedAction: "Consider if these drivers are truly independent or if one causes the other"
        });
      }
    }
  }
