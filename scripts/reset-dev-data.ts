...
const BACKEND_URL =
  process.env.BACKEND_URL || "https://uffp-backend.vercel.app";
...
const TEST_FORECASTS = [
  ...
  },
  {
    ...
    externalView: {
      ...
      confidence: "medium" as const,
      reasoning:
        "Major workplace shifts historically take 5-10 years to become standard",
      updatedAt: new Date().toISOString(),
    },
    createdAt: new Date(),
    updatedAt: new Date(),
...
async function deleteAllForecasts() {
  ...
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/api/forecasts?action=list&userId=${USER_ID}`,
    );
    if (!response.ok) {
    ...
    for (const forecast of forecasts) {
      ...
      if (!deleteResponse.ok) {
        console.warn(
          `  ⚠️  Failed to delete ${forecast.id}: ${deleteResponse.status}`,
        );
      } else {
        console.log(`  ✓ Deleted`);
      }
...
async function createTestForecasts() {
  ...
  for (const forecast of TEST_FORECASTS) {
    ...
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/forecasts?action=create`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to create forecast: ${response.status} - ${error}`,
        );
      }
...
async function main() {
  ...
  try {
    ...
    TEST_FORECASTS.forEach((f, i) => {
      ...
      console.log(
        `     - Base rate: ${f.externalView?.baseRate ? (f.externalView.baseRate * 100).toFixed(0) + "%" : "none"}`,
      );
    });
