...
export async function executeResearch(
  ...
  const promptTemplate = RESEARCH_PROMPT_TEMPLATES.find(p => p.id === promptId);
  if (!promptTemplate) {
...
function parseResearchResponse(response: string): {
  ...
  for (const line of lines) {
    ...
    if (currentSection === 'summary' && trimmed && !trimmed.match(/^[A-Z ]+:/)) {
      summary += ' ' + trimmed;
  ...
  return {
    ...
    keyFindings: keyFindings.length > 0 ? keyFindings : ['See full response for details'],
 ...
 */
export function getPromptTemplate(promptId: string): ResearchPromptTemplate | undefined {
